...
TEST_CASE("ProjectBuilder Constructor", "[ProjectBuilder]") {
    const std::string_view validFilename = "project_name";

    SECTION("Constructor initializes with valid filename") {
        vnd::ProjectBuilder builder(validFilename);
        REQUIRE(builder.getBuildFolderPath() == std::nullopt);
        REQUIRE(builder.getSrcFolderPath() == std::nullopt);
        REQUIRE(builder.getMainOutputFilePath() == std::nullopt);
    }

    SECTION("Constructor handles empty filename") {
        std::string_view emptyFilename;
        vnd::ProjectBuilder builder(emptyFilename);
        REQUIRE(builder.getBuildFolderPath() == std::nullopt);
        REQUIRE(builder.getSrcFolderPath() == std::nullopt);
        REQUIRE(builder.getMainOutputFilePath() == std::nullopt);
    }

    SECTION("Constructor handles special characters in filename") {
        std::string_view specialFilename = "proj@#$$%^^&";
        vnd::ProjectBuilder builder(specialFilename);
        REQUIRE(builder.getBuildFolderPath() == std::nullopt);
        REQUIRE(builder.getSrcFolderPath() == std::nullopt);
        REQUIRE(builder.getMainOutputFilePath() == std::nullopt);
    }
...
TEST_CASE("ProjectBuilder buildProject", "[ProjectBuilder]") {
    std::string_view validFilename = "project_name";
    vnd::ProjectBuilder builder(validFilename);

    SECTION("Build project succeeds when all folders and main file are created") {
        // Mock filesystem responses if needed (using dependency injection if possible)
        REQUIRE_NOTHROW(builder.buildProject());

        // Verify that the folders were created
        auto buildPath = builder.getBuildFolderPath();
        auto srcPath = builder.getSrcFolderPath();
        auto mainFilePath = builder.getMainOutputFilePath();

        REQUIRE(buildPath.has_value());
        REQUIRE(srcPath.has_value());
        REQUIRE(mainFilePath.has_value());
    }
...
TEST_CASE("ProjectBuilder Getter Methods", "[ProjectBuilder]") {
    std::string_view validFilename = "project_name.vn";
    vnd::ProjectBuilder builder(validFilename);

    SECTION("Get Build Folder Path returns correct value after build") {
    ...
    SECTION("Get Src Folder Path returns correct value after build") {
        builder.buildProject();
        auto srcPath = builder.getSrcFolderPath();
        REQUIRE(srcPath.has_value());
    ...
    SECTION("Get Main Output File Path returns correct value after build") {
        builder.buildProject();
        auto mainFilePath = builder.getMainOutputFilePath();
        REQUIRE(mainFilePath.has_value());
...
TEST_CASE("ProjectBuilder Corner Cases", "[ProjectBuilder]") {
    ...
    SECTION("Invalid characters in filename") {
        ...
        builder.buildProject();
        REQUIRE(builder.getMainOutputFilePath().has_value());
    }
